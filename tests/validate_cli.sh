...
if [ "$(cat "$WORKDIR/out")" != "$(printf 'fx_a::one\nfx_a::two\nfx_b::first\nfx_b::second\nfx_c::malformed_params')" ]; then
...
run --tags=alpha --no-color --json="$WORKDIR/tags_alpha.json"
...
run --tags=beta --no-color --json="$WORKDIR/tags_beta.json"
...
run --tags=nonexistent --no-color --json="$WORKDIR/tags_none.json"
...
run fx_b --fail-fast --no-color --json="$WORKDIR/failfast.json"
...
run fx_b::first --break --no-color --json="$WORKDIR/break.json"
...
run fx_b::first --debug --no-color --json="$WORKDIR/debug.json"
...
run fx_c::malformed_params --no-color --json="$WORKDIR/malformed.json"
